...
resource "aws_elb_target_group" "all-traffics-tg" {
  name        = "all-traffics-tg"
  port        = 31555
  protocol    = "tcp"
  target_type = ["0.0.0.0/0"]
  vpc_id      = aws_vpc.levelupvpc.id
}

#Security group for AWS NLB
...
resource "aws_security_group" "levelup-instance" {
  ...
  ingress {
    ...
    to_port         = 80
    protocol        = "tcp"
    security_groups = [aws_security_group.levelup-nlb-securitygroup.id]
  }
